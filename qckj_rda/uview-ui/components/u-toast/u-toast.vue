<template>
	<view class="u-toast"
		:class="[isShow ? 'u-show' : '', 'u-type-' + tmpConfig.type, 'u-position-' + tmpConfig.position]" :style="{
		zIndex: uZIndex
	}">
		<!-- <view class="u-icon-toast"> -->
		<u-icon v-if="tmpConfig.icon" class="u-icon" :name="iconName" :color="tmpConfig.type" width="60" height="60"
			size="36" top="1" />
		<!-- </view> -->
		<text class="u-text-toast" :style="{width: txtWidth}">{{tmpConfig.title}}</text>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			show(options) {
				...
				this.isShow = true;
				// 根据内容确定弹窗中text的宽度，最大宽度为600px
				let title = this.tmpConfig.title; // 内容
				let tempWidth = title.length * 34 + 6;
				if (tempWidth >= 600) tempWidth = 600;
				this.txtWidth = tempWidth+ 'rpx';
...
<style lang="scss" scoped>
	...
	.u-toast {
		...
		// 上右下左
		padding: 15rpx 20rpx 17rpx 15rpx;
	}
